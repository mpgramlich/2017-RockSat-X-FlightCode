...
/*-------------------------------------------------------------------
 ...
 -------------------------------------------------------------------*/
BYTE InitExtFlash(int driver_param)
{
   ...
   while ( get_cd( flashDriveNum) == 0 )
   {
      ...
      if ( get_cd() == 0 )
      {
      DEBUG_PRINT_NET( "No MMC/SD card detected.\r\n" );
      //getchar();
   }
#endif
      ...
      while ( get_wp(flashDriveNum) == 1 )
   {
      ...
      if ( get_wp() == 1 )
      {
         DEBUG_PRINT_NET( "SD/MMC Card is write-protected.\r\n" );
         //getchar();
   }
#endif
   ...
   */
   ...
   //sDEBUG_PRINT_NET(driveType, "SD/MMC");
   rv = f_mountfat( MMC_DRV_NUM, mmc_initfunc, driver_param );
#elif (defined USE_CFC)
